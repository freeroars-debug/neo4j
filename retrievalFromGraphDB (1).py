...
def embed_text(text: str) -> Optional[List[float]]:
    ...
    Returns a float list, or None if the call fails.

    Reads:
        EMBEDDING_MODEL  – Ollama model name (e.g. "nomic-embed-text")
        LLM_HOST / LLM_PORT – host and port for the Ollama server
    """
    llm_host  = os.getenv("LLM_HOST", "localhost")
    llm_port  = os.getenv("LLM_PORT", "11434")
    model     = os.getenv("EMBEDDING_MODEL", "nomic-embed-text")
    url       = f"http://{llm_host}:{llm_port}/api/embeddings"
    ...
    try:
        ...
        print(f"[embed_text] WARNING: embedding call failed – {exc}")
        return None
...
def _build_query_embedding(question: str, keywords: List[str]) -> Optional[List[float]]:
    """
    Build a single embedding that represents both the raw question and its
    extracted keywords (concatenated so the vector captures both forms).
    """
...
def fetch_relationships_for_nodes(
    ...
    # -- 2. Build a single query embedding (question + keywords) -------------
    query_vec = _build_query_embedding(question or " ".join(keywords), keywords)
...
def fetch_seed_relationships(
    ...
    """
    Scan ALL relationships in the graph and rank them by cosine similarity
    between the stored ``justification_embedding`` and an embedding of the user
    question + keywords.

    Falls back to a normalised lexical overlap score (with rel-type token
    weighting) when the embedding service is unavailable or a relationship
    ...
    similarity_threshold : minimum cosine score to keep (0.0 = keep all)
    """
    # -- 1. Pull ALL relationships with their stored embeddings ---------------
    cypher = """
    MATCH (a:Entity)-[r]->(b:Entity)
    RETURN
        elementId(r)                    AS rel_eid,
    ...
    for row in raw_rows:
        ...
        if query_vec and rel_vec and len(query_vec) == len(rel_vec):
            ...
            # Fallback: normalised lexical overlap.
            # rel-type tokens weighted x2 to preserve original intent.
            rt_parts = set((row["rel_type"] or "").lower().split("_"))
            just     = (row["rel_props"].get("justification") or "").lower()
            desc     = (row["rel_props"].get("description")   or "").lower()
            hits = sum(
                (2 if k in rt_parts else 0) + (1 if k in just else 0) + (1 if k in desc else 0)
                for k in kw_lower
            )
            score = hits / max(len(kw_lower) * 4, 1)
...
def callingFunction(question):
    ...
    rels_from_nodes = fetch_relationships_for_nodes(
        driver, node_eids=node_eids, keywords=kw, question=question, limit=300
    )

    seed_rels_text  = fetch_seed_relationships(driver, kw, question=question, limit=300)
